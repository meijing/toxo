class ProductType < ActiveRecord::Base
  ...
  def self.find_product_types(mark_id, category_id, product_type_id)
      ...
      where('(p.mark_id = ? or ? is null) and (p.category_id = ? or ? is null) and (p.product_type_id = ? or ? is null)', mark_id, mark_id, category_id, category_id, product_type_id, product_type_id).uniq.order(:name)
  end
end
