class HomeController < ApplicationController
  ...
  def sales
    ...
    if !@cat_id.nil?
      @products = Product.joins(:mark).where('sale = ? and marks.hidden = 0 and (outlet is null or outlet = 0) and (? is null or product_type_id=?) and category_id = ?  and (new_collection is null or new_collection = 0)', 1, @pt_id, @pt_id, @cat_id).order('marks.name, products.name')
    else
      @products = Product.joins(:mark).where('sale = ? and marks.hidden = 0 and (outlet is null or outlet = 0) and (new_collection is null or new_collection = 0)', 1).limit(15).order('random(),name')
    end
  end
